...
extension View {
    public func chatBubble(position: ChatBubble.TailPosition, cornerRadius: Double, color: Color) -> some View {
        self
            ...
            .padding(position.isLeading ? .leading : .trailing, CGFloat(cornerRadius / 2))
    }
    
    public func rotateChatBubble(position: ChatBubble.TailPosition) -> some View {
        switch position {
        case .trailingTop:
            return self.rotation3DEffect(Angle(degrees: 180), axis: (x: 0, y: 0, z: 0))
        case .trailingBottom:
            return self.rotation3DEffect(Angle(degrees: 180), axis: (x: 1, y: 0, z: 0))
        case .leadingTop:
            return self.rotation3DEffect(Angle(degrees: 180), axis: (x: 0, y: 1, z: 0))
        case .leadingBottom:
            return self.rotation3DEffect(Angle(degrees: 180), axis: (x: 0, y: 0, z: 1))
        }
    }
}
